...
static void from_json(const json& j, Config::Visuals& v)
{
	...
	read(j, "No scope zoom", v.noScopeZoom);
	...
	read<value_t::object>(j, "Smoke timer background", v.smokeTimerBG);
	...
	read<value_t::object>(j, "Molotov timer background", v.molotovTimerBG);
...
static void from_json(const json& j, Config::Misc& m)
{
	...
	read(j, "Blockbot", m.blockBot);
	read(j, "Blockbot Key", m.blockBotKey);
	read<value_t::object>(j, "Blockbot visualize", m.blockBotVisualize);
	...
	read(j, "Edge bug prediction amount", m.edgeBugPredAmnt);
	read(j, "Auto pixel surf", m.autoPixelSurf);
	read(j, "Auto pixel surf prediction amount", m.autoPixelSurfPredAmnt);
	...
	read(j, "Slowwalk velocity", m.slowwalkAmnt);
...
static void to_json(json& j, const Config::Misc& o)
{
	...
	WRITE("Blockbot Key", blockBotKey);
	WRITE("Blockbot visualize", blockBotVisualize);
	...
	WRITE("Edge bug prediction amount", edgeBugPredAmnt);
	WRITE("Auto pixel surf", autoPixelSurf);
	WRITE("Auto pixel surf prediction amount", autoPixelSurfPredAmnt);
	...
	WRITE("Slowwalk velocity", slowwalkAmnt);
...
static void to_json(json& j, const Config::Visuals& o)
{
	...
	WRITE("No scope zoom", noScopeZoom);
	...
	WRITE("Bullet tmpacts", bulletImpacts);
