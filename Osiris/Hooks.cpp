...
static int __fastcall sendDatagramHook(NetworkChannel* network, void* edx, void* datagram)
{
	...
	const float delta = std::max(0.f, (config->backtrack.fakeLatencyAmount / 1000.f));
...
static void __stdcall overrideView(ViewSetup* setup) noexcept
{
	...
	if (localPlayer) {
		...
		if (activeWeapon && activeWeapon->isSniperRifle() && localPlayer->isScoped()) {
			...
			if (!observed->isScoped() || config->visuals.noScopeZoom)
				setup->fov = 90.f;
		} else if (!once) {
	...
	if (localPlayer && ((localPlayer->isScoped() && config->visuals.noScopeZoom) || !localPlayer->isScoped()))
